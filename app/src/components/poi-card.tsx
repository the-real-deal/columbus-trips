...
export default function POICard({ className, showOrder, order, poi }: POICardProps) {
  return (
    <Card className={className}>
      <CardHeader className="flex flex-row items-center gap-3">
        {showOrder && <Badge><Hash className="inline" /> Tappa numero {order}</Badge>}
        <Badge><MapPin className="inline" /> {poi.location}</Badge>
      </CardHeader>
      <CardContent className="px-0">
        <div className="w-full h-70 overflow-hidden">
          <img src={poi.images[0]?.url || "/fallback.jpg"} className="object-center object-cover w-full h-full" />
        </div>
        <section className="px-10 grid grid-cols-3 py-3">
          <div className="py-5 col-span-2">
            ...
            <div className="grid gap-3 py-5">
              {poi.activities.map((activity, idx) => (
